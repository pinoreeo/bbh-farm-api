...
class HealthTreatmentController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $q = HealthTreatment::query()->with(['animal']);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $animal || $animal->life_status !== 'alive') {
            return response()->json([
                'message' => 'Catatan kesehatan hanya dapat dibuat untuk kambing yang masih hidup.',
            ], 422);
        }

        if ($animal->birth_date && $data['treatment_date'] < $animal->birth_date->toDateString()) {
            return response()->json([
                'message' => 'Tanggal perawatan tidak boleh lebih awal dari tanggal lahir kambing.',
            ], 422);
        }
        ...
        if ($exists) {
            return response()->json([
                'message' => 'Catatan kesehatan untuk kambing, jenis perawatan, produk, dan tanggal tersebut sudah ada.',
            ], 422);
        }
    ...
    public function update(Request $request, HealthTreatment $healthTreatment): JsonResponse
    {
        ...
        if ($exists) {
            return response()->json([
                'message' => 'Catatan kesehatan lain untuk kambing, jenis perawatan, produk, dan tanggal tersebut sudah ada.',
            ], 422);
        }
