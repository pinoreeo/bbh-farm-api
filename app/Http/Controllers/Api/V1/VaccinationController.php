...
class VaccinationController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $q = Vaccination::query()->with(['animal']);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $animal || $animal->life_status !== 'alive') {
            return response()->json([
                'message' => 'Data vaksinasi hanya dapat dibuat untuk kambing yang masih hidup.',
            ], 422);
        }

        if ($animal->birth_date && $data['vaccination_date'] < $animal->birth_date->toDateString()) {
            return response()->json([
                'message' => 'Tanggal vaksin tidak boleh lebih awal dari tanggal lahir kambing.',
            ], 422);
        }
        ...
        if ($exists) {
            return response()->json([
                'message' => 'Data vaksinasi untuk kambing, jenis vaksin, tanggal, dan produk tersebut sudah ada.',
            ], 422);
        }
    ...
    public function update(Request $request, Vaccination $vaccination): JsonResponse
    {
        ...
        if ($exists) {
            return response()->json([
                'message' => 'Data vaksinasi lain untuk kambing, jenis vaksin, tanggal, dan produk tersebut sudah ada.',
            ], 422);
        }
        ...
        return response()->json([
            'message' => 'Data vaksinasi berhasil diperbarui.',
