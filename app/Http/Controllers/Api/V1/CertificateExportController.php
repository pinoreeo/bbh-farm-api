...
class CertificateExportController extends Controller
{
    public function qr(Certificate $certificate, CertificateViewDataService $viewData): JsonResponse
    {
        if ($certificate->certificateType?->type_code !== 'BIBIT_UNGGUL') {
            return response()->json([
                'message' => 'QR code hanya tersedia untuk Sertifikat Bibit Unggul.',
            ], 422);
        }

        if (! $certificate->barcode_value) {
            return response()->json([
                'message' => 'QR code belum tersedia untuk sertifikat ini.',
            ], 422);
        }
    ...
    public function preview(
        ...
        if (! $certificate->certificateType) {
            return response()->json([
                'message' => 'Jenis sertifikat tidak ditemukan.',
            ], 422);
        }
        ...
        if (! $bladeView) {
            return response()->json([
                'message' => 'Jenis sertifikat tidak valid.',
            ], 400);
        }
    ...
    public function pdf(Certificate $certificate, CertificatePdfIntegrityService $pdfIntegrity): JsonResponse|BinaryFileResponse
    {
        try {
            ...
            return response()->json([
                'message' => 'File PDF sertifikat belum dapat dibuat. Pastikan data sertifikat sudah lengkap dan konfigurasi PDF benar.',
            ], 422);
        }
