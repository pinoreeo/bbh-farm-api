...
class WeightRecordController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $q = WeightRecord::query()->with(['animal']);
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (! $animal || $animal->life_status !== 'alive') {
            return response()->json([
                'message' => 'Catatan bobot hanya dapat dibuat untuk kambing yang masih hidup.',
            ], 422);
        }

        if ($animal->birth_date && $data['record_date'] < $animal->birth_date->toDateString()) {
            return response()->json([
                'message' => 'Tanggal timbang tidak boleh lebih awal dari tanggal lahir kambing.',
            ], 422);
        }
        ...
        if ($exists) {
            return response()->json([
                'message' => 'Catatan bobot untuk kambing dan tanggal tersebut sudah ada.',
            ], 422);
        }
        ...
        return response()->json([
            'message' => 'Catatan bobot berhasil disimpan.',
    ...
    public function update(Request $request, WeightRecord $weightRecord): JsonResponse
    {
        ...
        if (array_key_exists('record_date', $data)) {
            ...
            if ($exists) {
                return response()->json([
                    'message' => 'Catatan bobot lain untuk kambing dan tanggal tersebut sudah ada.',
                ], 422);
            }
        }
        ...
        return response()->json([
            'message' => 'Catatan bobot berhasil diperbarui.',
